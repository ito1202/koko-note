...
| FT-0005 | 2026-02-16 | flash-mem | Skill initializer failed because Python dependency was missing under managed Python policy. | Skill bootstrap stopped before validation. | `init_skill.py` requires `PyYAML`, but local Python lacked the module and default pip install was blocked by PEP 668 policy. | Before running Python-based skill tooling, verify dependencies and install user-scoped packages explicitly (`--user --break-system-packages` when required). | `ModuleNotFoundError: yaml` during skill init | team | active |
